...
def get_max_len_seq(dataset):
    ...
    max_len = 0
    for i in range(len(batch_features)):
        lenA = len(batch_features[i][0][0])  # protA's num of embeddings
        lenB = len(batch_features[i][1][0])  # protB's num of embeddings
        max_len = max(max_len, lenA, lenB)

    return max_len + 1  # add ProtBERT token
...
def get_dataset_split_stringDB_keep_ratio(poz_path, neg_path, protein_go_anno_pth, go_id_dict_pth, go_embed_pth, shuffle, aliases_path="", ratio=[0.8, 0.1, 0.1],
                                          stringDB=True, go_name_space_dict_pth="../datasets/STRING_dataset/go-terms/go_namespace_dict",
