...
elif model_organism in ['H', 'H_600-800', 'H_400-600'] :
    organism_file='H.sapiens'
    BATCH_SIZE = 512

elif model_organism =='D':
    organism_file='D.melanogaster'
    BATCH_SIZE = 512

elif model_organism =='M':
    organism_file='M.musculus'
    BATCH_SIZE = 512


if data_organism=='S':
    organism=4932
elif data_organism=='H':
    organism=9606
elif data_organism=='D':
    organism=7227
elif data_organism=='M':
    organism=10090

# model_name = f"model_STRING_{model_organism}_exp32_wd5e-05_dr0.1_best.pt"
# model_name = f"model_BioGRID_high_exp32_wd5e-05_dr0.1_best.pt"
# model_name = f"TransformerGO_Original_STRING_{model_organism}_best.pt"
# model_name = f"TransformerGO_Original_BioGRID_high_best.pt"
model_name = f"Node2vec_NN_STRING_{model_organism}_best.pt"
# model_name = f"Node2vec_NN_BioGRID_high_best.pt"

# model_path = f"model_results/Ablation/{organism_file}/{model_name}"
model_path = f"model_results/Comparative_Study/STRING_400-600/{organism_file}/{model_name}"
...
# # neg_path = f"{data_path}interaction-datasets/{organism}_biogrid_high_negative.txt"
# # poz_path = f"{data_path}interaction-datasets/{organism}_biogrid_high_positive.txt"
# embedding_pkl_path = f"{data_path}interaction-datasets/{organism}_esm2_embeddings.pkl"
# ------------------- BioGRID ------------------
...
_, _, test_set, all_set = get_dataset_split_stringDB_keep_ratio(
    poz_path, neg_path,
...
test_loader = data.DataLoader(all_set, batch_size=BATCH_SIZE, collate_fn=helper_collate, shuffle=False)
