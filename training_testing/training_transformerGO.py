...
# Print current date and time at program start
...
# model_id = "Node2vec_NN_STRING_H_600-800"
model_id = "TransformerGO_Original_STRING_H_600-800"
print(f'model id: {model_id}')
data_path = '../datasets/ADSLab_dataset/'

dataset_name = "STRING_H_600-800"

if dataset_name in ['STRING_S', 'STRING_S_Big', 'STRING_S_600', 'STRING_S_700']:
    BATCH_SIZE = 1024
    N_EPOCHS = 100
    organism = 4932
elif dataset_name in ['STRING_H' ,"STRING_H_Big", 'STRING_H_600', 'STRING_H_700', 'STRING_H_600-800']:
    BATCH_SIZE = 512
    N_EPOCHS = 50
    organism = 9606
...
neg_path = f"{data_path}interaction-datasets/{organism}.protein.negative.600-800.v12.0.txt"
poz_path = f"{data_path}interaction-datasets/{organism}.protein.links.600-800.v12.0.txt"
...
# Model
# model = TransformerGO_Scratch(EMB_DIM, 8, 3, 4*EMB_DIM, 0.2, using_esm2=True).to(device)
model = TransformerGO_Original(EMB_DIM, 8, 3, 4*EMB_DIM, 0.2).to(device)
...
for epoch in range(1, N_EPOCHS + 1):
    ...
    if epoch % EVAL_INTERVAL == 0:
        ...
        print(f'---> {epoch}epoch Testing Results: test_loss: {test_loss:.4f}, test_auc: {test_auc:.4f}')
